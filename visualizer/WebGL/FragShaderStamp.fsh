...
void main()
{
    ...
    if(iLayer > 254.9)
    {
        ...
        //if(bApplyTex<1)
        {
		    ...
		    mediump vec4 newTemplateColor = texture2D(sTemplateTexture, TexCoordOriginal);
		    
		    vNewColor = newTemplateColor;//vec4((iFactor+sFactor)*(BaseColor*(1.0-newTemplateColor.r) + TopColor*newTemplateColor.r), CurrentLayer);
		}    
